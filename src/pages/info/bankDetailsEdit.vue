<template>
    <el-card-module
            showHeader
            :title="`Редактирование рассчетного счета контрагента`"
            btnBack
            @goBack="goBack"
            showFooter
    ...
    >
        ...
        <div class="newBanks" v-else>
            <div class="newBanks__search" style="background: none;">
        ...
        <template slot="card-footer-actions" v-if="!is_loading_data">
            ...
            ><i class="mdi mdi-content-save"></i>Сохранить изменения</el-button>
...
<script>
    export default {
        ...
        watch: { },
        created(){
            this.getBank(this.$route.params.id).then(res => {
                this.model = res;
            }).then(() => {
        ...
        methods: {
            getBank(id){
                return new Promise((resolve, reject) => {
                    r.table("counterparties_bank_details").get(id).run(conn, (err, data) => {
                        if(err) reject(err);
                        resolve(data);
                    });
                })
            ...
            sendData(object){
                return new Promise((resolve, reject) => {
                    r.table("counterparties_bank_details").get(object.id).update(object).run(conn, (err, data) => {
                        if(err) reject(err);
            ...
            save(){
                ...
                this.sendData(this.model).then(result => {
                    this.$notify.success({
                        title: 'Успешно',
                        message: 'Банковский реквизит был измене',
                        duration: 1750
                    });
                }).catch(error => {
                    this.$notify.error({
